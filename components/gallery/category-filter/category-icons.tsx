import * as React from 'react';

interface IconProps {
  className?: string;
}

// Placeholder SVG icons - using example format, will be replaced with actual icons
const CookwareIcon = ({ className }: IconProps) => (
  <svg
    xmlns='http://www.w3.org/2000/svg'
    height='16px'
    viewBox='0 -960 960 960'
    width='16px'
    fill='currentColor'
    className={className}
  >
    <path d='M192-192q-81 0-136.5-55.5T0-384q0-79 53-135.5T192-576q69 0 121.5 44.5T380-420h47l-74-204h-65v-72h192v72h-50l17 48h204l-62-168H480v-72h108.8q27.2 0 42.7 12 15.5 12 25.5 36l70.04 192H760q81 0 140 55.5T959-384q0 79.84-56 135.92Q847-192 768-192q-69 0-122.5-44T579-348H380q-14 69-66.5 112.5T192-192Zm0-72q37 0 68.5-21.5T306-348H192v-72h114q-11-37-44-60.5T192-504q-50 0-85 35.5T72-384q0 50 35 85t85 35Zm312-156h76q5-23 14.5-44t24.5-40H473l31 84Zm263.5 156q49.5 0 85-35.5t35.5-84.01q0-49.49-38.5-87.99Q811-510 755-502q-1 0 0 0l41 114-68 24-40-109q-19.05 17-29.52 40Q648-410 648-384q0 50 35 85t84.5 35ZM192-381Zm570 5Z' />
  </svg>
);

const EverydayCarryIcon = ({ className }: IconProps) => (
  <svg
    xmlns='http://www.w3.org/2000/svg'
    height='16px'
    viewBox='0 -960 960 960'
    width='16px'
    fill='currentColor'
    className={className}
  >
    <path d='M192-192q-81 0-136.5-55.5T0-384q0-79 53-135.5T192-576q69 0 121.5 44.5T380-420h47l-74-204h-65v-72h192v72h-50l17 48h204l-62-168H480v-72h108.8q27.2 0 42.7 12 15.5 12 25.5 36l70.04 192H760q81 0 140 55.5T959-384q0 79.84-56 135.92Q847-192 768-192q-69 0-122.5-44T579-348H380q-14 69-66.5 112.5T192-192Zm0-72q37 0 68.5-21.5T306-348H192v-72h114q-11-37-44-60.5T192-504q-50 0-85 35.5T72-384q0 50 35 85t85 35Zm312-156h76q5-23 14.5-44t24.5-40H473l31 84Zm263.5 156q49.5 0 85-35.5t35.5-84.01q0-49.49-38.5-87.99Q811-510 755-502q-1 0 0 0l41 114-68 24-40-109q-19.05 17-29.52 40Q648-410 648-384q0 50 35 85t84.5 35ZM192-381Zm570 5Z' />
  </svg>
);

const BicycleIcon = ({ className }: IconProps) => (
  <svg
    xmlns='http://www.w3.org/2000/svg'
    height='16px'
    viewBox='0 -960 960 960'
    width='16px'
    fill='currentColor'
    className={className}
  >
    <path d='M192-192q-81 0-136.5-55.5T0-384q0-79 53-135.5T192-576q69 0 121.5 44.5T380-420h47l-74-204h-65v-72h192v72h-50l17 48h204l-62-168H480v-72h108.8q27.2 0 42.7 12 15.5 12 25.5 36l70.04 192H760q81 0 140 55.5T959-384q0 79.84-56 135.92Q847-192 768-192q-69 0-122.5-44T579-348H380q-14 69-66.5 112.5T192-192Zm0-72q37 0 68.5-21.5T306-348H192v-72h114q-11-37-44-60.5T192-504q-50 0-85 35.5T72-384q0 50 35 85t85 35Zm312-156h76q5-23 14.5-44t24.5-40H473l31 84Zm263.5 156q49.5 0 85-35.5t35.5-84.01q0-49.49-38.5-87.99Q811-510 755-502q-1 0 0 0l41 114-68 24-40-109q-19.05 17-29.52 40Q648-410 648-384q0 50 35 85t84.5 35ZM192-381Zm570 5Z' />
  </svg>
);

const FoodIcon = ({ className }: IconProps) => (
  <svg
    xmlns='http://www.w3.org/2000/svg'
    height='16px'
    viewBox='0 -960 960 960'
    width='16px'
    fill='currentColor'
    className={className}
  >
    <path d='M192-192q-81 0-136.5-55.5T0-384q0-79 53-135.5T192-576q69 0 121.5 44.5T380-420h47l-74-204h-65v-72h192v72h-50l17 48h204l-62-168H480v-72h108.8q27.2 0 42.7 12 15.5 12 25.5 36l70.04 192H760q81 0 140 55.5T959-384q0 79.84-56 135.92Q847-192 768-192q-69 0-122.5-44T579-348H380q-14 69-66.5 112.5T192-192Zm0-72q37 0 68.5-21.5T306-348H192v-72h114q-11-37-44-60.5T192-504q-50 0-85 35.5T72-384q0 50 35 85t85 35Zm312-156h76q5-23 14.5-44t24.5-40H473l31 84Zm263.5 156q49.5 0 85-35.5t35.5-84.01q0-49.49-38.5-87.99Q811-510 755-502q-1 0 0 0l41 114-68 24-40-109q-19.05 17-29.52 40Q648-410 648-384q0 50 35 85t84.5 35ZM192-381Zm570 5Z' />
  </svg>
);

const ApparelIcon = ({ className }: IconProps) => (
  <svg
    xmlns='http://www.w3.org/2000/svg'
    height='16px'
    viewBox='0 -960 960 960'
    width='16px'
    fill='currentColor'
    className={className}
  >
    <path d='M192-192q-81 0-136.5-55.5T0-384q0-79 53-135.5T192-576q69 0 121.5 44.5T380-420h47l-74-204h-65v-72h192v72h-50l17 48h204l-62-168H480v-72h108.8q27.2 0 42.7 12 15.5 12 25.5 36l70.04 192H760q81 0 140 55.5T959-384q0 79.84-56 135.92Q847-192 768-192q-69 0-122.5-44T579-348H380q-14 69-66.5 112.5T192-192Zm0-72q37 0 68.5-21.5T306-348H192v-72h114q-11-37-44-60.5T192-504q-50 0-85 35.5T72-384q0 50 35 85t85 35Zm312-156h76q5-23 14.5-44t24.5-40H473l31 84Zm263.5 156q49.5 0 85-35.5t35.5-84.01q0-49.49-38.5-87.99Q811-510 755-502q-1 0 0 0l41 114-68 24-40-109q-19.05 17-29.52 40Q648-410 648-384q0 50 35 85t84.5 35ZM192-381Zm570 5Z' />
  </svg>
);

const FlowersIcon = ({ className }: IconProps) => (
  <svg
    xmlns='http://www.w3.org/2000/svg'
    height='16px'
    viewBox='0 -960 960 960'
    width='16px'
    fill='currentColor'
    className={className}
  >
    <path d='M192-192q-81 0-136.5-55.5T0-384q0-79 53-135.5T192-576q69 0 121.5 44.5T380-420h47l-74-204h-65v-72h192v72h-50l17 48h204l-62-168H480v-72h108.8q27.2 0 42.7 12 15.5 12 25.5 36l70.04 192H760q81 0 140 55.5T959-384q0 79.84-56 135.92Q847-192 768-192q-69 0-122.5-44T579-348H380q-14 69-66.5 112.5T192-192Zm0-72q37 0 68.5-21.5T306-348H192v-72h114q-11-37-44-60.5T192-504q-50 0-85 35.5T72-384q0 50 35 85t85 35Zm312-156h76q5-23 14.5-44t24.5-40H473l31 84Zm263.5 156q49.5 0 85-35.5t35.5-84.01q0-49.49-38.5-87.99Q811-510 755-502q-1 0 0 0l41 114-68 24-40-109q-19.05 17-29.52 40Q648-410 648-384q0 50 35 85t84.5 35ZM192-381Zm570 5Z' />
  </svg>
);

const BeautyIcon = ({ className }: IconProps) => (
  <svg
    xmlns='http://www.w3.org/2000/svg'
    height='16px'
    viewBox='0 -960 960 960'
    width='16px'
    fill='currentColor'
    className={className}
  >
    <path d='M192-192q-81 0-136.5-55.5T0-384q0-79 53-135.5T192-576q69 0 121.5 44.5T380-420h47l-74-204h-65v-72h192v72h-50l17 48h204l-62-168H480v-72h108.8q27.2 0 42.7 12 15.5 12 25.5 36l70.04 192H760q81 0 140 55.5T959-384q0 79.84-56 135.92Q847-192 768-192q-69 0-122.5-44T579-348H380q-14 69-66.5 112.5T192-192Zm0-72q37 0 68.5-21.5T306-348H192v-72h114q-11-37-44-60.5T192-504q-50 0-85 35.5T72-384q0 50 35 85t85 35Zm312-156h76q5-23 14.5-44t24.5-40H473l31 84Zm263.5 156q49.5 0 85-35.5t35.5-84.01q0-49.49-38.5-87.99Q811-510 755-502q-1 0 0 0l41 114-68 24-40-109q-19.05 17-29.52 40Q648-410 648-384q0 50 35 85t84.5 35ZM192-381Zm570 5Z' />
  </svg>
);

const SwimsuitIcon = ({ className }: IconProps) => (
  <svg
    xmlns='http://www.w3.org/2000/svg'
    height='16px'
    viewBox='0 -960 960 960'
    width='16px'
    fill='currentColor'
    className={className}
  >
    <path d='M192-192q-81 0-136.5-55.5T0-384q0-79 53-135.5T192-576q69 0 121.5 44.5T380-420h47l-74-204h-65v-72h192v72h-50l17 48h204l-62-168H480v-72h108.8q27.2 0 42.7 12 15.5 12 25.5 36l70.04 192H760q81 0 140 55.5T959-384q0 79.84-56 135.92Q847-192 768-192q-69 0-122.5-44T579-348H380q-14 69-66.5 112.5T192-192Zm0-72q37 0 68.5-21.5T306-348H192v-72h114q-11-37-44-60.5T192-504q-50 0-85 35.5T72-384q0 50 35 85t85 35Zm312-156h76q5-23 14.5-44t24.5-40H473l31 84Zm263.5 156q49.5 0 85-35.5t35.5-84.01q0-49.49-38.5-87.99Q811-510 755-502q-1 0 0 0l41 114-68 24-40-109q-19.05 17-29.52 40Q648-410 648-384q0 50 35 85t84.5 35ZM192-381Zm570 5Z' />
  </svg>
);

// Default/fallback icon
const DefaultIcon = ({ className }: IconProps) => (
  <svg
    xmlns='http://www.w3.org/2000/svg'
    height='16px'
    viewBox='0 -960 960 960'
    width='16px'
    fill='currentColor'
    className={className}
  >
    <path d='M192-192q-81 0-136.5-55.5T0-384q0-79 53-135.5T192-576q69 0 121.5 44.5T380-420h47l-74-204h-65v-72h192v72h-50l17 48h204l-62-168H480v-72h108.8q27.2 0 42.7 12 15.5 12 25.5 36l70.04 192H760q81 0 140 55.5T959-384q0 79.84-56 135.92Q847-192 768-192q-69 0-122.5-44T579-348H380q-14 69-66.5 112.5T192-192Zm0-72q37 0 68.5-21.5T306-348H192v-72h114q-11-37-44-60.5T192-504q-50 0-85 35.5T72-384q0 50 35 85t85 35Zm312-156h76q5-23 14.5-44t24.5-40H473l31 84Zm263.5 156q49.5 0 85-35.5t35.5-84.01q0-49.49-38.5-87.99Q811-510 755-502q-1 0 0 0l41 114-68 24-40-109q-19.05 17-29.52 40Q648-410 648-384q0 50 35 85t84.5 35ZM192-381Zm570 5Z' />
  </svg>
);

export type CategoryIconComponent = React.ComponentType<IconProps>;

export const categoryIconMap: Record<string, CategoryIconComponent> = {
  cookware: CookwareIcon,
  'everyday-carry': EverydayCarryIcon,
  bicycle: BicycleIcon,
  food: FoodIcon,
  apparel: ApparelIcon,
  flowers: FlowersIcon,
  beauty: BeautyIcon,
  swimsuit: SwimsuitIcon,
};

export function getCategoryIcon(slug: string): CategoryIconComponent {
  return categoryIconMap[slug] || DefaultIcon;
}

